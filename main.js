...
const botstart = async (ctx) =>{
    try{
        ...
        let text = "*©Share Your Contact In Order To Start Using The Bot. This Is Just A Phone Number Verification\n\n*_⚠️Note : We Will Not Share Your Details With Anyone_"
        bot.telegram.sendMessage(ctx.from.id,text,{parse_mode:'Markdown',reply_markup:{keyboard:[[{text:"💢 Share Contact",request_contact:true}]],resize_keyboard: true}})
    }catch(e){
        console.log(e)
senderr(e)
    }
...
bot.hears('👫 Invite', async (ctx)=>{
    try{
        ...
        let text = "*🙌  User = "+ctx.from.first_name+"\n\n🙌 Refer Link = https://Gacashback.in/Lifafa?id="+ctx.from.id+"\n\n🚀 Invite And Earn: "+admin[0].ref.toFixed(3)+" "+curr+" *"
        ctx.replyWithMarkdown(text)
    }catch(e){
        console.log(e)
senderr(e)
...
bot.hears('🗂️ Wallet', async (ctx) =>{
    try{
        ...
        ctx.replyWithMarkdown("*✏️ Now Send Your INR Wallet Address To Use It For Future Withdrawals\n\n*_⚠️ This Wallet Will Be Used For Future Withdrawals !!_",{reply_markup:{keyboard:[
            ['🔙 Back']
...
bot.hears('💵 Withdraw',async (ctx) =>{
    ...
    ctx.replyWithMarkdown("*📤 Enter Amount To Withdraw*",{reply_markup:{keyboard:[
        ['🔙 Back']
    ],resize_keyboard:true}})
...
bot.action('continue',async (ctx) =>{
    try{
        ...
        var url = 'https://earningarea.in/nogetway.php?key='+mkey+'&token='+swg+'&mid='+mid+'&amo='+amo.toString()+'&num='+wallet+'&com='+comment;
